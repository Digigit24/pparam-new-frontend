...
(function($) {
    ...
    ////////////////////////////////////////////////////
    // 01. Loader Disabled (Perfectly Removed)
    ...
    windowOn.on('load', function() {
        ...
        }, 100); // Reduced delay since loader is gone
    });

    ////////////////////////////////////////////////////
    // 00. Critical Visibility & Backgrounds
    ...
    // Refresh ScrollTrigger after any DOM changes or image loads
    ...
    function tp_text_invert() {
        const split = new SplitType(".tp_text_invert", {
            types: "lines"
        });
    ...
    ///////////////////////////////////////////////////
    // 38. has_fade_anim
    if ($(".has_fade_anim").length > 0) {
        gsap.utils.toArray(".has_fade_anim").forEach((item) => {
            let fade_offset = item.getAttribute("data-fade-offset") || 50,
                duration_value = item.getAttribute("data-duration") || 1.15,
                fade_direction = item.getAttribute("data-fade-from") || "bottom",
                onscroll_value = item.getAttribute("data-on-scroll") || 1,
                delay_value = item.getAttribute("data-delay") || 0.15,
            ...
            let anim_setting = {
                ...
                delay: delay_value,
                x: (fade_direction == "left" ? -fade_offset : (fade_direction == "right" ? fade_offset : 0)),
                y: (fade_direction == "top" ? -fade_offset : (fade_direction == "bottom" ? fade_offset : 0)),
            };

            if (onscroll_value == 1) {
                anim_setting.scrollTrigger = {
                    trigger: item,
                    start: 'top 85%',
                };
            }
            gsap.from(item, anim_setting);
        });
    }
    ...
    gsap.utils.toArray(".tp-text-perspective").forEach(SplitTypeLine => {
        const delay_value = parseFloat(SplitTypeLine.getAttribute("data-delay") || 0.5);
        const tl = gsap.timeline({
            scrollTrigger: {
                trigger: SplitTypeLine,
                start: 'top 85%',
                duration: 1.5,
                scrub: false,
                markers: false,
        ...
        const itemSplitted = new SplitType(SplitTypeLine, {
            types: "lines"
        });
        gsap.set(SplitTypeLine, {
            perspective: 400
        });
    ...
    if ($(".tp_fade_anim").length > 0) {
        gsap.utils.toArray(".tp_fade_anim").forEach((item) => {
            let tp_fade_offset = item.getAttribute("data-fade-offset") || 50,
                tp_duration_value = item.getAttribute("data-duration") || 1.15,
    ...
    if ($('.has-scale-image').length > 0) {
        $('.has-scale-image').each(function() {
            ...
            hiddenImageDiv.css({
                ...
                'background-color': bgColor // AdaugÄƒ culoarea de fundal
            });
    ...
    function rotate_text() {
        let rotateText = document.querySelector(".tp-rotate-text");
        if (rotateText) {
            let headingTitle = new SplitType(rotateText, {
                types: "chars"
            });
    ...
    td_anim_reveal.forEach(areveal => {
        ...
        areveal.split = new SplitType(areveal, {
    ...
    if ($(".text-anim").length) {
        ...
        animatedTextElements.forEach((element) => {
            let animationSplitType = new SplitType(element, {
                types: "chars, words"
            });
            gsap.from(animationSplitType.chars, {
                duration: 1,
                delay: delayValue,
                x: translateXValue,
                autoAlpha: 0,
